...
/************************************************************************
 * received.php															*
 * function: displays the endorsements a nation has received.			*
...
function get_received() {
	...
	foreach ($_GET as $name=>$value) $$name=$value;
	$nation_db=display_to_url($nation);
	$sql = "create temporary table ".$prefix."temp_received
				select endorser as nation,0 as is_returned from ".$prefix."endorsements where endorsee='$nation';";
	mysql_query($sql);
	$sql = "update ".$prefix."temp_received a join ".$prefix."endorsements b on a.nation=b.endorsee 
				set a.is_returned=1 where b.endorser='$nation';";
	mysql_query($sql);
	...
	$sql = "select count(*) from ".$prefix."temp_received $where;";
	$count = mysql_query($sql);
	$count= mysql_fetch_array($count);
	$count=$count[0];
	$sql = "select count(*) from ".$prefix."temp_received $unwhere;";
	$total = mysql_query($sql);
	$total= mysql_fetch_array($total);
	$total=$total[0];
	$sql = "select * from ".$prefix."temp_received $where order by nation limit $st,$limit;";
	$res = mysql_query($sql);
	//echo mysql_error();
...
<p><a href=".">Index</a> -&gt; <a href="region.php?region=<?=$region_db?>"><?=$region?></a>
 -&gt; <strong><?=$nation?> -&gt; Endorsements received</strong></p>
...
if ($nation) { 
	if ($region) {
?>
<h2>Endorsements received by: <?=$nation?> in <?=$region?></h2>
<p><?=($returned)?"<a href='?nation=$nation_db'>":"<strong>"?>All endorsements received by <?=$nation?><?=($returned)?'</a>':'</strong>'?> | 
...
<hr/>
<p>
Of <strong><?=$total?></strong> endorsements, <strong><?=$nation?></strong> has returned <strong><?=$nreturned?></strong> or
...
?>
	<tr>
		...
		<td style='background:<?=$color[$is_returned]?>'><a href="given.php?nation=<?=$nation?>" title="Other endorsees"><?=($is_returned)?'Yes':'No'?></a></td>
	</tr>
<?php
			}
		} else {
